...
@section('content')
    <div class="container">
        <h5 class="mb-3">Form P423 Keluarga</h5>
        <form action="{{ route('kg-p423.store') }}" method="post" class="mb-5">
            @csrf
            <div class="row">
                {{-- ID Master Fasilitas Kesehatan --}}
                <div class="col-sm-3 mb-3">
                    <label>Tenaga Kesehatan Terdekat<span class="text-danger">*</span></label>
                    <select name="id_master_tenkes" class="form-control" required>
                        <option value="" selected disabled>-- Pilih Tenaga Kesehatan --</option>
                        @foreach ($masternakes as $p)
                            <option value="{{ $p->id }}">{{ $p->tenaga_kesehatan }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-sm-3">
        ...
        <div class="card shadow mb-4">
            <div class="card-header py-3">
                <h6 class="m-0 font-weight-bold text-primary">Data Keluarga P423</h6>
            </div>
            <div class="card-body">
                    ...
                    <table class="table display" id="dataTable" width="100%" cellspacing="0">
                        ...
                        <tbody>
                            ...
                            @foreach ($data as $item)
                                <tr>
                                    <td>{{ $item->masterNakes->tenaga_kesehatan }}</td>
                                    <td>
                                        {{ $item->jarak }} Km
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        ...
                                        <button class="btn btn-warning btn-sm" data-toggle="modal"
                                            data-target="#modalEditKeluargaP423">
                                            <i class="bi bi-pencil-square"></i> Edit
                                        </button>

                                        <!-- Tombol Hapus -->
                                        <form action="{{ route('kg-p423.destroy', $item->id) }}" method="POST"
        ...
        {{-- Modal Edit Keluarga P423 --}}
        <div class="modal fade" id="modalEditKeluargaP423" aria-labelledby="modalEditKeluargaLabel" aria-hidden="true">
            <div class="modal-dialog modal-xl">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        @foreach ($data as $item)
                            <form action="{{ route('kg-p423.update', $item->id) }}" method="POST">
                                @csrf @method('PUT')
                                <div class="row">
                                    <div class="col-sm-3 mb-3">
                                        <label>Tenaga Kesehatan Terdekat<span class="text-danger">*</span></label>
                                        <select name="id_master_nakes" class="form-control" required>
                                            @foreach ($masternakes as $p)
                                                <option value="{{ $p->id }}"
                                                    {{ $p->id == $item->id_master_tenkes ? 'selected' : '' }}>
                                                    {{ $p->tenaga_kesehatan }}
                                                </option>
                                            @endforeach
                                        </select>
